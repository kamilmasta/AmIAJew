...
class PeselValidatorImpl() : PeselValidator {

    private val PESEL = ByteArray(11)

    override fun validate(pesel: String?): Boolean {

        return when {
            pesel == null -> false
            pesel.length != 11 -> false
            else -> {
                for (i in 0..10) {
                    PESEL[i] = java.lang.Byte.parseByte(pesel.substring(i, i + 1))
                }
                checkSum() && checkMonth() && checkDay()
            }
        }
    }
